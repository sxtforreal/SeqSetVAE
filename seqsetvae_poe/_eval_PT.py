...
def compute_posterior_moments_dataset(
    ...
    for i, batch in enumerate(dataloader):
        ...
        # Iterate sets and accumulate moments
        for sets in all_sets:
            for s in sets:
                if hasattr(model, "set_encoder"):
                    encoder = model.set_encoder
...
def collect_mu_var_heatmaps(
    ...
    encoder = (
        model.set_encoder
        if hasattr(model, "set_encoder")
        else getattr(model, "setvae", None)
    )
    if encoder is None:
        return np.zeros((0, 0), dtype=np.float32), np.zeros((0, 0), dtype=np.float32)
    for s in sets:
...
def collect_recon_events(
    ...
    orig = (
        np.concatenate(orig_cat, axis=0)
        if orig_cat
        else np.zeros((0, 0), dtype=np.float32)
    )
    recon = (
        np.concatenate(recon_cat, axis=0)
        if recon_cat
        else np.zeros((0, 0), dtype=np.float32)
    )
    return orig, recon
...
def main():
    ...
    ap.add_argument(
        ...
        required=True,
        type=str,
        help="Path to pretraining checkpoint (.ckpt)",
    )
    ap.add_argument(
        "--ckpt_type", type=str, choices=["auto", "setvae", "poe"], default="auto"
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=50,
        help="Number of batches to estimate KL stats",
    )
    ap.add_argument(
        "--num_vis_samples",
        type=int,
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.01,
        help="KL threshold for active units",
    )
    ...
    active_ratio = (
        float(np.mean(kl_dim > args.active_threshold)) if kl_dim.size > 0 else 0.0
    )
    ...
    # 1b) Posterior moments across dataset for collapse diagnostics
    ...
    with open(dim_csv, "w", newline="") as fcsv:
        ...
        writer.writerow(["dim", "kl", "mu_abs_mean", "mu_mean", "mu_std", "var_mean", "var_std", "collapsed"])
    ...
    dm_vis.setup()
    dl_vis = (
        dm_vis.val_dataloader() if args.split == "valid" else dm_vis.test_dataloader()
    )

    recon_results: List[Dict[str, float]] = []
    vis_count = 0
    # Per-sample CSV and correlations
    ...
    for i, batch in enumerate(dl_vis):
        ...
        sample_rows.append([
            i,
            metrics.get("kl_total", float("nan")),
            metrics.get("nn_l2_mean", float("nan")),
            metrics.get("nn_l2_median", float("nan")),
            metrics.get("nn_l2_p95", float("nan")),
            metrics.get("dir_cosine_mean", float("nan")),
            metrics.get("mag_mae", float("nan")),
            metrics.get("mag_rmse", float("nan")),
            metrics.get("mag_corr", float("nan")),
            metrics.get("scale_ratio", float("nan")),
            metrics.get("chamfer_l2_mean", float("nan")),
            metrics.get("active_ratio", float("nan")),
        ])
    ...
    # Correlations between per-sample KL and recon metrics
    ...
    with open(sample_csv, "w", newline="") as fcsv:
        writer = csv.writer(fcsv)
        writer.writerow(["sample_index", "kl_total", "nn_l2_mean", "nn_l2_median", "nn_l2_p95", "dir_cosine_mean", "mag_mae", "mag_rmse", "mag_corr", "scale_ratio", "chamfer_l2_mean", "active_ratio"])
