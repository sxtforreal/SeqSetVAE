...
def build_sequences(
    ...
    if normalize:
        ...
        if len(train_df) == 0:
            raise RuntimeError("No training rows found in SOFA CSV for the provided split_dir")
        mean_vec = train_df[feature_names].astype(float).mean(axis=0).to_numpy(dtype=np.float32)
        std_raw = train_df[feature_names].astype(float).std(axis=0).to_numpy(dtype=np.float32)
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--label_csv", required=True, help="Outcome CSV with mortality labels (oc)")
    ap.add_argument("--split_dir", required=True, help="Root directory containing train/valid/test per-patient Parquets")
    ap.add_argument("--feature_set", default="total", choices=["total", "subscores", "all"], help="Which SOFA features to use")
    ap.add_argument("--normalize", action="store_true", help="Z-normalize features using training set statistics")
    ap.add_argument("--max_len", type=int, default=None, help="Optional cap on sequence length (truncate head)")
